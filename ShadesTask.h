...
// This includes  x 2 relays for windows shades. each one up other down. never both relays active to avoid programming mode of shades
...
private:
    ...
    virtual void OnUpdate(uint32_t deltaTime)
    {
        ...
        case PS_DECREASING:
          ...
          Serial.println(" decr C"+String(currentPosition[_window])+" T"+String(targetPosition[_window]));
          if (currentPosition[_window]<=targetPosition[_window]) 
          { 
        ...
        case PS_INCREASING:
          currentPosition[_window]+=1;
          Serial.println(" incr C"+String(currentPosition[_window])+" T"+String(targetPosition[_window]));
          if (currentPosition[_window]>=targetPosition[_window]) 
          { 
            positionState[_window]=PS_STOPPED;
            stopWindow(_window);  
